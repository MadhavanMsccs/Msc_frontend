...
const ProvideExamMarks = () => {
  ...
  const fetchStudents = async (className) => {
    try {
      const response = await axios.get(`https://msc-backend-3.onrender.com/api/v1/examMarks/${className}`);
  ...
  const handleProvideMarks = async () => {
    try {
      ...
      await axios.post('https://msc-backend-3.onrender.com/api/v1/examMarks/provide', newMark);
