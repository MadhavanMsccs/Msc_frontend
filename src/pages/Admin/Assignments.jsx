...
const Assignments = () => {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await axios.get('https://msc-backend-3.onrender.com/api/v1/assignments/getall');
  ...
  const handleAddAssignment = async (e) => {
    ...
    if (newAssignment.title.trim() !== '' && newAssignment.description.trim() !== '' && newAssignment.class.trim() !== '' && newAssignment.deadline.trim() !== '') {
      try {
        const response = await axios.post('https://msc-backend-3.onrender.com/api/v1/assignments', newAssignment);
  ...
  const handleDeleteAssignment = async (id) => {
    try {
      await axios.delete(`https://msc-backend-3.onrender.com/api/v1/assignments/${id}`);
