...
const Students = () => {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://msc-backend-3.onrender.com/api/v1/students/getall');
  ...
  const handleAddStudent = async (e) => {
    ...
    if (newStudent.name.trim() !== '' && newStudent.registrationNumber.trim() !== '' && newStudent.email.trim() !== '' && newStudent.className.trim() !== '') {
      try {
        const response = await axios.post('https://msc-backend-3.onrender.com/api/v1/register/student', newStudent);
